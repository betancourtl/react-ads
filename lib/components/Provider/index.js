...
// After ads get added to the queue, trigger a debounced callback function.
...
function (_Component) {
  ...
  function Provider(props) {
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "display", function (cb) {
      if (_this.state.serviceEnabled) {
        // Do not use the queue anymore.
        // just display the ads.
        cb();
        return;
      }

      _this.defineSlotQueue.enqueue(cb);

      setTimeout(function () {
        while (_this.peek) {
          var cbFn = _this.defineSlotQueue.dequeue();
    ...
    _this.defineSlotQueue = new _Queue.default();
    ...
    _this.pubSub.on('enableServices', function () {
    ...
    _this.pubSub.on('defineSlot', function (slot) {
      return console.log('defineSlot', slot.getSlotElementId());
    });
